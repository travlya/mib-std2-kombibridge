...
ExboxServiceListener {
    ...
    public static volatile String navSecondary = null;
    public static volatile String navTertiary = null;
    public static volatile String navQuaternary = null;
    // AAtoKombi now-playing-in-media injection. ShmemMediaReader sets these (real AA track) from the
    // patched GAL MediaPlaybackStatusEndpoint. setStationInfoForMirrorLink() shows them in place of
    // the static "Android Auto" label while AA is connected and no nav guidance is active.
    ...
    public BAPEntity init(BAPStageInitializer bAPStageInitializer) {
        INSTANCE = this;
        MIBLogger.getInstance().debug("CurrentStationInfo.init (AAtoKombi shadow instantiated)");
    ...
    private void setStationInfoForMirrorLink(CurrentStationInfo_Status currentStationInfo_Status) {
        int n;
        String string;
        int connType = this.getSmartphoneIntegrationService().getSmarphoneIntegrationActiveConnectionType();
        switch (connType) {
            case 1: {
                string = "MirrorLink\u2122";
                n = 72;
        ...
        if (PROBE_LAYOUT && connType == 3) {
            ...
            currentStationInfo_Status.ti_Type = 0;
            currentStationInfo_Status.quaternaryInformation.setContent("Q4");
            currentStationInfo_Status.qi_Type = 0;
            currentStationInfo_Status.primaryInformation.setContent("P1");
            currentStationInfo_Status.pi_Type = 0;
